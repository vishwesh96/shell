...
std::unordered_set<int> bg_process;
std::unordered_set<int> bg_process_exit;
       ...
       tokens = tokenize(line,token_size);
        ...
        if(strcmp(tokens[0],"cd") == 0){
          ...
          getbg(tokens,*token_size,"no display");
        	...
        	for(unordered_set::iterator it = bg_process.begin();it!=bg_process.end();it++){						//empty the bg_process so that they dont print completed
